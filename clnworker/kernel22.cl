...
__kernel void kernel22(__global uint* dBufIn, __global char* plain_key, __global uint* bPart, __global char* dValidKey) {
	int gid = get_global_id(0);
	int gidy = get_global_id(1);
	int gidz = get_global_id(2);
	int t, msg_pad, current_pad, stop, mmod, ai, aI, lsNCF;
	uint i, ulen, item, total, nn, key;
	uint W[80], A, B, C, D, E, F, G, H, T1, T2;
	uint lPart[8];
	uint num_keys = dBufIn[1];
	const int length = 6;
	int rlInLen = dBufIn[2];
	int inLen = rlInLen + length;
	int inVec = inLen - length;
	int id2 = dBufIn[3];
	int id3 = dBufIn[4];
	uint c;
	bool solved = 0;
	const char charset[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789mL#";
	char shifter;
	char local_key[90];
	...
	if (id2 == 0 && id3 == 0) {
		...
		for (ai = 0; ai < length; ai++) {
			dValidKey[ai] = charset[64];
		}
	}
	local_key[inVec + 1] = charset[id2];
	local_key[inVec + 2] = charset[id3];
	local_key[inVec + 3] = charset[gidz];
	local_key[inVec + 4] = charset[gidy];
	local_key[inVec + 5] = charset[gid];
	ulen = inLen;
...
0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2 };
	for (lsNCF = 0; lsNCF < 62; lsNCF++) {
		local_key[inVec] = charset[lsNCF];

		msg_pad = 0;
		c = 0;
		lPart[0] = H0; lPart[1] = H1; lPart[2] = H2; lPart[3] = H3; lPart[4] = H4; lPart[5] = H5; lPart[6] = H6; lPart[7] = H7;
...
#pragma unroll
		...
		if (v) {
			...
			solved = 1;
			barrier(CLK_GLOBAL_MEM_FENCE);
			for (ai = 0; ai < length; ai++) { dValidKey[ai] = local_key[inVec + ai]; }
			for (ai = 0; ai < 8; ai++) { bPart[ai] = lPart[ai]; }
			break;
		}
		if (c > 27) { printf("\n[k22]str:%d [ls:%d|i1:%d|i2:%d|gz:%d|gy:%d|gx:%d] '%c%c%c ... %c%c%c%c%c%c%c%c%c%c'", c / 4, lsNCF, id3, id2, gidz, gidy, gid, local_key[0], local_key[1], local_key[2], local_key[inVec - 4], local_key[inVec - 3], local_key[inVec - 2], local_key[inVec - 1], local_key[inVec], local_key[inVec + 1], local_key[inVec + 2], local_key[inVec + 3], local_key[inVec + 4], local_key[inVec + 5]); }
	}
}
