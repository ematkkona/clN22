...
std::string wuID;
std::string setDeviceId = "0";
static std::int32_t timeToWait;
static std::string execclN22 = "clN22.exe";										// clN22 executable
static std::int32_t retryLmt = 6;												// number of retries allowed when encountering errors
static std::string BaseUrl = "http://kkona.xyz:8080/workitem";					// url for clN22-manager, format "http://<host>:<port>/workitem"
...
void signalHandler(int signum) {
	std::cout << endl << "Releasing workitem ...";
}

int main(int argc, char* argv[])
{
	std::cout << "clN22-wproxy v0.410-020220 (c)2019-2020 EM~ eetu@kkona.xyz" << endl << endl;
	if (argc == 2 && argv[1] != "") {
		setDeviceId = argv[1];
	...
	std::int32_t countAtt = 0;
	std::int32_t relayStat = 0;
	...
	while (proceed) {
					...
					if (workitem["result"] == zwRes) {
						...
						while (countAtt < 1000) {
							auto r = cpr::Put(cpr::Url{ ToWorkUrl + workId },
								...
								cpr::Header{ { "Content-Type", "application/json" } });
							if (r.status_code == 200) {
								std::cout << endl << wOffer << " Got work:" << workIn << endl;
								//"[r200]AttemptNo:" << countAtt << endl << "[prefix]" << workIn << endl << "[time]" << wOffer << endl;
								proceed = 1;
								countAtt = 1000;
							}
		...
		if (workId == "" || workIn == "") {
			...
			if (proceed) {
				...
				std::string argsFor22i = execclN22 + " \"" + workIn + "\" \"" + "work/" + workId + ".out\" setdev " + setDeviceId; //id0=290x id1=vega64
				...
				if (!execExit) {
					...
					if (resultOut != "" && proceed == 1) {
						...
						auto r = cpr::Put(cpr::Url{ ToWorkUrl + workId },
							...
							cpr::Header{ { "Content-Type", "application/json" } });
						if (r.status_code == 200) {
							if (resultOut != "userexit") {
								std::cout << endl << "'" << resultOut << "'" << " All done. Sleeping for 1 second..." << endl;
		...
		if (proceed == 0 || errorCond == 0) {
			std::cout << "Sleeping for "<< timeToWait/1000 << " seconds..." << endl << endl;
			proceed = 1;
